#!/usr/bin/env node
/**
 * Show consecutive Yes/No streaks from btc-updown.json.
 *
 * Config via .env:
 *   BTC_UPDOWN_OUTPUT   - JSON file path (default: btc-updown.json)
...
function getConfig(): { inputPath: string; minStreak: number | null } {
  const inputPath = process.env.BTC_UPDOWN_OUTPUT?.trim() || 'btc-updown.json';
